    ...
    userChanged: {
      ...
      subscribe: withFilter(
        ...
        (message, _params, { tenant, logger }, _info) => {
          logger.logInfo(
            `📨   Message received from ${topics.USER_CHANGED}: ${JSON.stringify(message)}`,
            '[Message_Received]',
            true
          )
          logger.logInfo(
            `📨   Message tenant id =  ${envelope.getTenantId(message).toUpperCase()}; Context tenant id = ${tenant?.id?.toUpperCase()}`,
            '[Message_Tenant_Check]',
            true
          )

...
const isAuthenticated = rule({ cache: 'contextual' })((_parent, _args, context) => !!context?.externalUser?.id)

const isAdmin = rule({ cache: 'contextual' })(
  (_parent, _args, { externalUser }, _info) => includes(admin, externalUser.role) || includes(globalAdmin, externalUser.role)
)

const canViewDashboard = rule({ cache: 'contextual' })((_parent, _args, context, _info) =>
  checkForPermission([viewDashboard], context)
)
...
const checkForPermission = async (permissions, { externalUser }) => {
  try {
    const rights = (
      await prisma()
        .user.findFirst({ where: { ExternalId: externalUser?.id } })
        .UserRights({ include: { Right: true } })
    )?.map(x => x?.right?.name)

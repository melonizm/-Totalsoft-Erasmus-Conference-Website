...
export const requiredFile = Validator(x =>
  x && x.fileId && x.fileName ? Success : Failure(ValidationError(i18next.t('Validations.FileIsMandatory')))
)

export const integer = Validator(x => (Number.isInteger(x) ? Success : Failure(ValidationError(i18next.t('Validations.Integer')))))
